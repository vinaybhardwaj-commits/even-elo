...
const STREAM_IDS = ["insurance_denial"];
...
export default function BillingInputPage() {
  ...
  useEffect(() => {
    fetch("/api/streams?team=Billing")
      .then((r) => r.json())
      .then((j) => {
        if (!j.ok) {
  ...
  return (
    <>
      ...
      <main className="max-w-[1400px] mx-auto px-8 py-8">
        <div className="flex items-end justify-between mb-6">
          <div>
            <div className="flex items-center gap-2 text-xs text-stone-500 mb-2">
              ...
              <span>/</span>
              <span className="text-stone-900 font-medium">Billing</span>
            </div>
            <h1 className="text-2xl font-semibold tracking-tight">Insurance denial</h1>
            <p className="text-sm text-stone-500 mt-1">
              1 stream · default no-event. Mark cases where insurance denied. Optional reason.
            </p>
          </div>
        </div>
        ...
        {streams.length > 0 && (
          <CaseTable streams={streams} expectedTeam="Billing" expectedTeamLabel="Billing" />
        )}
      </main>
    </>
  );
