...
export async function POST(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (req[0].hospital_id) {
    await sql`UPDATE physician_engagements SET status='resigned', status_reason='Resignation processed', updated_at=NOW() WHERE physician_id = ${req[0].physician_id}::uuid AND hospital_id = ${req[0].hospital_id}::uuid AND status='active'`;
  } else {
    await sql`UPDATE physician_engagements SET status='resigned', status_reason='Resignation processed', updated_at=NOW() WHERE physician_id = ${req[0].physician_id}::uuid AND status='active'`;
  }
