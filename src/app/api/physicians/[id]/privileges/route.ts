...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { hospital_code, procedure_or_specialty, granted_date, basis } = body ?? {};
  ...
  const inserted = (await sql`
    INSERT INTO privileges (
      physician_id, hospital_id, procedure_or_specialty, granted_date, granted_by, basis
    ) VALUES (
      ${id}::uuid, ${hosp[0].id}::uuid, ${String(procedure_or_specialty).trim()},
      ${granted_date}, ${actor.profileId}::uuid, ${finalBasis}
    )
    RETURNING id::text AS id, physician_id::text AS physician_id, procedure_or_specialty, granted_date, basis, created_at
  `) as Array<Record<string, unknown>>;
